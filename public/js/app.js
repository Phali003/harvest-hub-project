// Harvest Hub Frontend Application
class HarvestHub {
    constructor() {
        this.cart = [];
    ...
    setupEventListeners() {
        ...
        // Forms
        document.getElementById('findFoodBtn').addEventListener('click', () => this.handleFindFood());
        
        // Search functionality
        // Navigation search bar - food search only
        document.getElementById('searchInput').addEventListener('input', (e) => this.handleFoodSearch(e.target.value));
        ...
        const navSearchBtn = document.querySelector('#searchInput').parentElement.querySelector('button');
        if (navSearchBtn) {
    ...
    loadMockData() {
        ...
        this.categories = [
            { id: 1, name: 'Vegetables', icon: '🥬', description: 'Fresh local vegetables' },
            ...
            { id: 18, name: 'Juices', icon: '🧃', description: 'Fresh pressed juices' },
            ...
            { id: 23, name: 'Mushrooms', icon: '🍄', description: 'Wild & cultivated' },
            { id: 24, name: 'Flowers', icon: '🌸', description: 'Fresh cut flowers' },
            { id: 25, name: 'Sauces', icon: '🥫', description: 'Homemade sauces' },
            { id: 26, name: 'Soups', icon: '🍲', description: 'Fresh prepared soups' },
            { id: 27, name: 'Salads', icon: '🥗', description: 'Fresh prepared salads' },
            { id: 28, name: 'Oils', icon: '🫒', description: 'Cold-pressed oils' },
            { id: 29, name: 'Vinegars', icon: '🍷', description: 'Artisan vinegars' },
            { id: 30, name: 'Spices', icon: '🧂', description: 'Fresh ground spices' }
        ];
        ...
        this.products = [
            ...
            },
            {
                ...
                producer_name: "Green Valley Farm",
                category: "Vegetables",
                image: "https://images.unsplash.com/photo-1447175008436-170170753a5d?w=300&h=300&fit=crop"
            },
            {
                id: 4,
                name: "Bell Peppers",
                ...
                producer_name: "Green Valley Farm",
                category: "Vegetables",
                image: "https://images.unsplash.com/photo-1525607551316-5a9eeaab95ba?w=300&h=300&fit=crop"
            },
            {
                id: 5,
                name: "Cucumbers",
            ...
            },
            {
                ...
                producer_name: "Sunny Side Dairy",
                category: "Dairy",
                image: "https://images.unsplash.com/photo-1486297678162-eb2a19b0a9d5?w=300&h=300&fit=crop"
            },
            {
                id: 45,
    ...
    renderProducers() {
        ...
        grid.innerHTML = this.producers.map(producer => `
            <div class="bg-white rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow">
                <div class="text-center mb-4">
                    ...
                    <h3 class="font-semibold text-gray-900 text-lg">${producer.business_name}</h3>
                    <p class="text-gray-600 text-sm">${producer.address_city}, ${producer.address_state}</p>
                </div>
                
                <p class="text-gray-700 text-sm mb-4">${producer.description}</p>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm text-gray-500">${producer.total_reviews} reviews</span>
                </div>
                
                <button onclick="app.viewProducerProducts(${producer.id})" class="w-full mt-4 bg-primary text-white py-2 px-4 rounded-md hover:bg-primary/90 transition-colors">
                    View Products
                </button>
            </div>
        `).join('');
    ...
    initAuth() {
        ...
        if (showRegister) {
            ...
            console.error('❌ [AUTH] showRegister element NOT FOUND! Cannot add click listener.');
        }
    ...
    initRegistrationFlow() {
        ...
        roleOptions.forEach(option => {
            option.addEventListener('click', () => {
                ...
                roleOptions.forEach(opt => opt.classList.remove('border-primary', 'border-secondary'));
    ...
    showRegistrationForm(role) {
        ...
        // Update role display
        if (role === 'customer') {
            if (roleIcon) roleIcon.className = 'fas fa-shopping-cart text-primary mr-3 text-xl';
            if (roleTitle) roleTitle.textContent = 'Customer Account';
            if (roleDescription) roleDescription.textContent = 'Browse and buy fresh local products';
            if (producerFields) producerFields.classList.add('hidden');
            if (businessNameField) businessNameField.required = false;
        } else if (role === 'producer') {
            if (roleIcon) roleIcon.className = 'fas fa-store text-secondary mr-3 text-xl';
            if (roleTitle) roleTitle.textContent = 'Producer Account';
            if (roleDescription) roleDescription.textContent = 'Sell your farm products to local customers';
    ...
    async handleRegistration(form) {
        ...
        // Collect form data
        const fields = ['registerFirstName', 'registerLastName', 'registerEmail', 'registerPhone', 
                       'registerPassword', 'registerConfirmPassword', 'businessName', 'businessDescription'];
        ...
        // Validation
        if (!data.registerFirstName || !data.registerLastName || !data.registerEmail || 
            !data.registerPhone || !data.registerPassword || !data.registerConfirmPassword) {
        ...
        if (selectedRole === 'producer' && !data.businessName) {
            this.showNotification('Business name is required for producer accounts', 'error');
            return;
        }
        ...
        try {
            ...
            console.log('Registration attempt:', { ...requestData, password: '[HIDDEN]' });
    ...
    loadNearbyProducers(location) {
        ...
        // Add loading state
        nearbyContainer.innerHTML = '<div class="text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-primary"></i><p class="mt-2 text-gray-600">Finding producers near you...</p></div>';
    ...
    handleFoodSearch(query) {
        ...
        console.log(`Found ${filteredProducts.length} products for query: "${query}"`, filteredProducts);
        ...
        if (filteredProducts.length > 0) {
            this.showNotification(`Found ${filteredProducts.length} products matching "${query}"`);
        } else {
    ...
    async handleLocationSearch(location) {
        try {
            ...
            if (data && data.length > 0) {
                ...
                this.showNotification(`Could not find "${location}". Showing nearby results.`, 'error');
                this.showMapSection(location);
            }
        } catch (error) {
            console.error('Geocoding error:', error);
            this.showNotification(`Could not search location "${location}". Showing nearby results.`, 'error');
    ...
    async showRealMapSection(locationData) {
        ...
        this.initializeMap(locationData.lat, locationData.lon, locationData.displayName);
        ...
        mapContainer.innerHTML = `
            <div class="w-full h-full bg-gradient-to-br from-green-100 to-green-200 flex items-center justify-center relative">
                ...
                <div class="text-center z-10">
                    ...
                    <p class="text-sm text-gray-600">Lat: ${lat.toFixed(4)}, Lon: ${lon.toFixed(4)}</p>
        ...
        const producersWithCoordinates = this.producers.map((producer, index) => {
            ...
            switch(producer.address_city) {
                ...
                case 'Nakuru':
                    ...
                    baseLon = 36.0800 + (Math.random() - 0.5) * 0.1;
                    break;
                case 'Nairobi':
                ...
                case 'Limuru':
                    ...
                    baseLon = 36.6420 + (Math.random() - 0.5) * 0.1;
                    break;
                case 'Nanyuki':
            ...
            const distance = this.calculateDistance(searchLat, searchLon, baseLat, baseLon);
        ...
        container.innerHTML = producers.map(producer => `
            <div class="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-start space-x-3">
                    ...
                    <div class="flex-1">
                        <h4 class="font-semibold text-gray-900">${producer.business_name}</h4>
                        <p class="text-sm text-gray-600 mb-2">${producer.description}</p>
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-4 text-sm text-gray-500">
                                <span><i class="fas fa-map-marker-alt mr-1"></i>${producer.distance.toFixed(1)} miles away</span>
                                <span><i class="fas fa-star text-yellow-400 mr-1"></i>${producer.rating}</span>
                            </div>
                            <button onclick="app.viewProducerProducts(${producer.id})" class="text-primary hover:text-primary/80 text-sm font-medium">
                                View Products
                            </button>
                        </div>
                    </div>
